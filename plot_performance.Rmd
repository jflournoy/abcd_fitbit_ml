...
```{r}
scores <- fread('model_train_test_scores.csv')
scores[, c('cv_iter', 'time') := list(
  ifelse(cv_iter == '', time, cv_iter),
  ifelse(cv_iter == '', '', time))]
...
scores_summary[, mid := 1:.N]
scores_summary_l <- melt(scores_summary, id.vars = c('mode', 'summary', 'time', 'mid'))
...
model_summary <- unique(scores_summary_w[, c('mode', 'summary', 'time', 'mid')])
model_summary[, time := ifelse(time == '', '', paste0('_', time))]
model_summary[, c('summary', 'time') := list(paste0(summary, time), NULL)]
model_summary_l <- melt(model_summary, id.vars = 'mid')
...
model_summary_w_l <- melt(model_summary_w, id.vars = 'mid')

...
aggregate_and_widen <- function(d){
  ...
  d_all_weeks_l_l[, variable := paste('pt',
                                      variable, 
                                      variable.1, sep = '_')]
  ...
  d_w <- merge(
    merge(d_day_w, d_each_week_w, 
          by = c('subjectkey', 'weekno', 'eventname', 'interview_age', 'sex'), 
          all = TRUE), 
    d_all_weeks_w, 
    by = c('subjectkey', 'eventname', 'interview_age', 'sex'),
    all = TRUE)
  return(d_w)
}
...
data_wide <- merge(cbcl, merge(fbdpas_w, fbdss_w, by = c(idvars, 'weekno', 'daynum'), all = TRUE), by = idvars, all = TRUE)
has_cols <- c('has_activity', 'has_sleep', 'has_clinical')
data_wide[, (has_cols) :=
